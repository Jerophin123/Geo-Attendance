...
def get_db_connection():
    conn = mariadb.connect(
        user="jero",          # Replace with your MariaDB username
        password="1168",      # Replace with your MariaDB password
        host="192.168.0.189", # Replace with your MariaDB host
        port=3306,            # Default MariaDB port
        database="sample"     # Replace with your MariaDB database name
    )
    return conn
...
def init_db():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS User (
            ...
            employee_type ENUM('onsite', 'offsite') NOT NULL
        )
    ''')
    conn.commit()
    conn.close()
...
def add_employee():
    ...
    cursor.execute('SELECT * FROM User WHERE email = %s OR employee_id = %s', (email, employee_id))
    ...
    if existing_user:
        ...
        cursor.execute('''
            INSERT INTO User (firstname, lastname, email, employee_id, password, employee_type) 
            VALUES (%s, %s, %s, %s, %s, %s)
